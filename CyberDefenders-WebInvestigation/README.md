...
*   **Cách làm:** Phân tích lưu lượng giao tiếp để xác định IP có dấu hiệu gửi lượng request bất thường.
...
*   **Thao tác thực hiện:** Chúng ta đã biết địa chỉ IP của attacker là `111.224.250.131`, sử dụng AbuseIP cho kết quả địa chỉ IP này tới từ Shijiazhuang.
*   **Bằng chứng:**
...
*   **Thao tác thực hiện:** Dựa vào hình ảnh trên ta có thể thấy các câu lệnh tấn công SQL Injection đang lợi dụng lỗ hổng trên script `search.php` để thực hiện tấn công.
*   **Bằng chứng:**
...
*   **Thao tác thực hiện:** Sử dụng filter: `ip.src==111.224.250.131 and http and frame contains "schema"` để tìm những truy vấn liên quan tới database. Ta có 1 request nghi ngờ là frame số 1520. Khi copy request và đem đi decode ta thu về được URL: `/search.php?search=book' UNION ALL SELECT NULL,CONCAT(0x7178766271,JSON_ARRAYAGG(CONCAT_WS(0x7a76676a636b,schema_name)),0x7176706a71) FROM INFORMATION_SCHEMA.SCHEMATA-- -`. Đây chính là request để đọc các database của web server.
*   **Bằng chứng:**
...
*   **Thao tác thực hiện:** Ở Wireshark, chúng ta sử dụng filter `ip.src==111.224.250.131 and http and frame contains "schema"`. Chúng ta chú ý tới frame số 1548. Ở frame 1548, follow HTTP stream: Như chúng ta thấy, table chứa dữ liệu người dùng là `customers`.
*   **Bằng chứng:**
...
*   **Thao tác thực hiện:** Sử dụng filter: `http.request.method==POST` ta thu được directory đã discover bởi attacker là `/admin/`.
*   **Bằng chứng:**
...
*   **Thao tác thực hiện:** Sử dụng filter: `http.request.method==POST`. Khi kiểm tra tới frame số 88699, ta phát hiện được attacker sử dụng username:admin và password:admin123!. Response là 302 cho thấy credentials này là tồn tại.
*   **Bằng chứng:**
...
*   **Thao tác thực hiện:** Filter: `http.request.method==POST`. Khi kiểm tra tới frame số 88757: Follow HTTP Stream của frame này cho ta thấy được attacker đã sử dụng 1 script là `NVri2vhp.php` để thực thi thành công reverse shell về máy chủ attacker.
*   **Bằng chứng:**
